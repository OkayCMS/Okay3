...
class CatalogLogic
{
    ...
    public function getPriceFilter($catalogType, $objectId = null)
    {
        ...
        if (isset($currentPrices['min']) && isset($currentPrices['max']) && $currentPrices['max'] != '' && $currentPrices['min'] != '') {
            $resultPrice = $currentPrices;
        }

        if (empty($resultPrice) && $priceFilter['price_range']['min'] != '' && $priceFilter['price_range']['max'] != '') {
    ...
    public function getPrices(array &$filter, $catalogType, $objectId = null)
    {
        ...
        } elseif ($priceFilter['price_range']['min'] != '' && $priceFilter['price_range']['max'] != '') {
        ...
        setcookie("price_filter", serialize($priceFilter), time()+3600*24*1, "/");
    ...
    private function getPriceFromStorage($catalogType, $objectId = null)
    {
        ...
        if (isset($_COOKIE['price_filter'])) {
            $priceFilter = unserialize($_COOKIE['price_filter']);
        }
