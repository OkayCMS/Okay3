...
class FeaturesEntity extends Entity
{
    ...
    public function updateFeatureCategories($featureId, array $categoriesIds)
    {
        ...
        if (is_array($categoriesIds)) {
            ...
            // todo для customQuery нужно сделать bindValues
            $this->db->customQuery("DELETE `pv` FROM `__products_features_values` AS `pv`
                            ...
                            WHERE 
                                `fv`.`feature_id`={$featureId} 
                                AND `pc`.`position`=(SELECT MIN(`pc2`.`position`) FROM `__products_categories` AS `pc2` WHERE `pc`.`product_id`=`pc2`.`product_id`) 
                                AND `pc`.`category_id` NOT IN (" . implode(',', $categoriesIds) . ")");
        } else {
            // Если не прислали категорий, тогда удаляем все значения этого свойства для товаров
            $this->db->customQuery("DELETE `pv` FROM `__products_features_values` AS `pf`
                              INNER JOIN `__features_values` AS `fv` ON `pf`.`value_id`=`fv`.`id` AND `fv`.`feature_id` = {$featureId}");
        }
    }
