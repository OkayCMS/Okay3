...
class FeaturesValuesEntity extends Entity
{
    ...
    protected function filter__feed($state)
    {
            $this->select->where("f.yandex=:feed_filter AND pf.product_id IN (
                SELECT 
                    distinct(p.id)
                FROM ok_variants v 
            ...
            ->bindValue('feed_filter', (int)$state);
    }
    ...
    public function deleteProductValue($productsIds, $valuesIds = null, $featuresIds = null)
    {
        ...
        // todo сделать для customQuery bindValues
        $this->db->customQuery("DELETE `pf`
