...
class LanguagesEntity extends Entity
{
        ...
        if ($langId) {
            ...
            if ($entitiesLangInfo = $languagesCore->getEntitiesLangInfo()) {
                foreach ($entitiesLangInfo as $entityLangInfo) {
                    $this->db->customQuery('INSERT INTO __lang_' . $entityLangInfo->langTable . ' (' . implode(',', $entityLangInfo->fields) . ', ' . $entityLangInfo->object . '_id, lang_id)
            ...
            if (isset($this->mainLanguage) && !empty($this->mainLanguage)) {
                ...
                if (!empty($settings)) {
                    foreach ($settings as $s) {
                        $this->db->customQuery("REPLACE INTO `__settings_lang` SET 
                                                    `lang_id`={$this->db->escape($langId)},
                                                    `param`={$this->db->escape($s->param)},
                                                    `value`={$this->db->escape($s->value)}
                                                    ");
                    }
                }
            } else {
                $this->db->customQuery("UPDATE `__settings_lang` SET `lang_id`={$this->db->escape($langId)}");
            }
        }
        $this->initLanguages();
        return $langId;
    ...
    {
        $this->db->customQuery("SHOW TABLES LIKE '%__lang\_%'");
        $tables = [];
