...
class BrandController extends AbstractController
{
    ...
    public function render(
        ...
        if (($currentBrandsIds = $chpuFilterLogic->getCurrentBrands($filtersUrl)) === false || !empty($currentBrandsIds)) {
            return false;
        }

        // Если нашли фильтр по свойствам, кидаем 404
        if (($currentFeatures = $chpuFilterLogic->getCurrentCategoryFeatures($filtersUrl)) === false || !empty($currentFeatures)) {
            return false;
        }
        ...
        if ((!empty($filter['price']) && $filter['price']['min'] != '' && $filter['price']['max'] != '') || !empty($filter['other_filter'])) {
