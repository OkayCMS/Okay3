...
class CategoryController extends AbstractController
{
    ...
    public function render(
        ...
        ChpuFilterLogic $chpuFilterLogic,
        ...
        $chpuFilterLogic->setFiltersUrl($filtersUrl);
        ...
        $chpuFilterLogic->setCategory($category);
        $this->categoryFeatures = $chpuFilterLogic->getCategoryFeatures();
        
        if (($currentBrandsIds = $chpuFilterLogic->getCurrentBrands($filtersUrl)) === false) {
            return false;
        }
        
        if (($currentOtherFilters = $chpuFilterLogic->getCurrentOtherFilters($filtersUrl)) === false) {
            return false;
        }
        
        if (($currentPage = $chpuFilterLogic->getCurrentPage($filtersUrl)) === false) {
            return false;
        }
        
        if (($currentFeatures = $chpuFilterLogic->getCurrentCategoryFeatures($filtersUrl)) === false) {
            return false;
        }
        
        if (($currentSort = $chpuFilterLogic->getCurrentSort($filtersUrl)) === false) {
            return false;
        }

        $chpuFilterLogic->changeLangUrls($filtersUrl);
        ...
        $metaArray = $chpuFilterLogic->getMetaArray($filtersUrl);
        ...
        foreach ($featuresValues as $featureValue) {
            if(isset($this->categoryFeatures[$featureValue->feature_id])) {
                $chpuFilterLogic->setCategoryFeatureValue($featureValue);
        ...
        $this->setHeaderLastModify(max($lastModify));
        //lastModify END
        ...
        $filterAutoMeta = $chpuFilterLogic->getFilterAutoMeta($filtersUrl);
        $this->design->assign('filter_meta', $filterAutoMeta);

        $this->design->assign('set_canonical', $chpuFilterLogic->isSetCanonical($filtersUrl));
        ...
        $this->design->assign('sort_canonical', $chpuFilterLogic->getSortCanonical());
