...
class Languages
{
    ...
    // todo Если пустой lang_label в запросе, то ф-ия langId() может вернуть предыдущий язык сессии,
    // при переключении на основной язык. Так как сперва ищем langId в $_GET, потом lang_label,
    // и если этих параметров нету а в сессии есть язык(предыдущий по факту), то вернём его.
    // В этом случае должна быть вызвана ф-ия set_langId() до get_lang_link();
