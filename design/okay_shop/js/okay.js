...
$(function(){
    ...
    /* Мега меню */
    if ($(window).width() < 768) {
        $('.categories_menu__link').click(function(e) {
            e.preventDefault();
            var itemActive = $(this).closest('li');

            if ($(window).width() > 767) {
                var contActive = $(this).closest('ul');
                $(contActive).children().not(itemActive).removeClass('open');
            }
            $(itemActive).toggleClass('open');
        });
    } else {
