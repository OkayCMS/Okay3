            ...
            <div class="okay_list_boding okay_list_module_version hidden-md-down">
                ...
                {if !empty($module->params->version) && $module->params->version != $module->version}
                    {if $module->params->version > $module->version}
                        <button type="button" class="fn_update_module btn btn_border_blue btn_big hint-top-middle-t-info-s-small-mobile  hint-anim" data-hint="{$btr->module_need_update} {$module->params->version}">{include 'svg_icon.tpl' svgId='refresh_icon'} {$module->params->version}</button>
                    {else}
