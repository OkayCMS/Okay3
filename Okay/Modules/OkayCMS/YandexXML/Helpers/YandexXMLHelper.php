...
namespace Okay\Modules\OkayCMS\YandexXML\Helpers;
...
class YandexXMLHelper
{
    ...
    private function getSubSelect($feedId, $uploadCategories = [])
    {
        ...
        $sql->cols([
            ...
            ->where('p.visible')
            ->where("p.id NOT IN (SELECT entity_id FROM " . YandexXMLRelationsEntity::getTable() . " WHERE feed_id = :feed_id AND entity_type = 'product' AND include = 0)")
            ->where("(p.id IN (SELECT entity_id FROM " . YandexXMLRelationsEntity::getTable() . " WHERE feed_id = :feed_id AND entity_type = 'product' AND include = 1) OR
                           p.brand_id IN (SELECT entity_id FROM " . YandexXMLRelationsEntity::getTable() . " WHERE feed_id = :feed_id AND entity_type = 'brand')
                           {$categoryFilter})")
        ...
        if (!$this->settings->get('okaycms__yandex_xml__upload_without_images')) {
            $sql->where('p.main_image_id != \'\' AND p.main_image_id IS NOT NULL');
        }

        if ($this->settings->get('okaycms__yandex_xml__no_export_without_price')) {
            $sql->where('v.price > 0');
        }
    ...
    public function getItem($product, $addVariantUrl = false)
    {
        ...
        $result['store']['data'] = $this->settings->get('okaycms__yandex_xml__store') ? 'true' : 'false';
        $result['delivery']['data'] = $this->settings->get('okaycms__yandex_xml__delivery_disallow') ? 'true' : 'false';
        $result['pickup']['data'] = $this->settings->get('okaycms__yandex_xml__pickup') ? 'true' : 'false';
        $result['adult']['data'] = $this->settings->get('okaycms__yandex_xml__adult') ? 'true' : 'false';
        $result['manufacturer_warranty']['data'] = $this->settings->get('okaycms__yandex_xml__has_manufacturer_warranty') ? 'true' : 'false';
        ...
        if ($this->settings->get('okaycms__yandex_xml__sales_notes')) {
            $result['sales_notes']['data'] = $this->feedHelper->escape($this->settings->get('okaycms__yandex_xml__sales_notes'));
        }
