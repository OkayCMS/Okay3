        ...
        <offers>
            {foreach $products as $p}
                {foreach $p->variants as $v}
                    {if $settings->okaycms__yandex_xml__no_export_without_price == 1 && $v->price == 0}{continue}{/if}
                    ...
                    <offer id="{$v->id}" available="{if $v->stock > 0 || $v->stock === null}true{else}false{/if}">
                        ...
                        <store>{if $settings->okaycms__yandex_xml__store}true{else}false{/if}</store>
                        <delivery>{if $settings->okaycms__yandex_xml__delivery_disallow}true{else}false{/if}</delivery>
                        <pickup>{if $settings->okaycms__yandex_xml__pickup == 1}true{else}false{/if}</pickup>
                        <adult>{if $settings->okaycms__yandex_xml__adult == 1}true{else}false{/if}</adult>
                        ...
                        {if $settings->okaycms__yandex_xml__sales_notes}
                            <sales_notes>{$settings->okaycms__yandex_xml__sales_notes|strip_tags|escape}</sales_notes>
                        {/if}
                        ...
                        <description>{if $settings->okaycms__yandex_xml__use_full_description_in_upload}{$p->description|strip_tags|escape}{else}{$p->annotation|strip_tags|escape}{/if}</description>
                        <manufacturer_warranty>{if $settings->okaycms__yandex_xml__has_manufacturer_warranty}true{else}false{/if}</manufacturer_warranty>
